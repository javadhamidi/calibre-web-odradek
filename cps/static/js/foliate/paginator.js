...
export class Paginator extends HTMLElement {
    ...
    constructor() {
        ...
        // Scroll-mode overscroll: detect wheel/touch at section boundaries
        // and trigger section transitions when the user keeps scrolling
        let overscrollAccum = 0
        let overscrollTimer = null
        let overscrollTransitioning = false
        const triggerOverscroll = (forward) => {
            if (overscrollTransitioning) return
            overscrollTransitioning = true
            overscrollAccum = 0
            // Fade out, transition, fade in for visual continuity
            this.#container.style.transition = 'opacity 120ms ease-out'
            this.#container.style.opacity = '0.3'
            const go = forward ? this.next() : this.prev()
            Promise.resolve(go).then(() => {
                this.#container.style.transition = 'opacity 200ms ease-in'
                this.#container.style.opacity = '1'
                setTimeout(() => {
                    this.#container.style.transition = ''
                    overscrollTransitioning = false
                }, 220)
            })
        }
        const onWheel = e => {
            ...
            const atTop = this.start <= 0
            if ((e.deltaY > 0 && atBottom) || (e.deltaY < 0 && atTop)) {
                overscrollAccum += Math.abs(e.deltaY)
                clearTimeout(overscrollTimer)
                overscrollTimer = setTimeout(() => overscrollAccum = 0, 250)
                if (overscrollAccum > 30) {
                    triggerOverscroll(e.deltaY > 0)
                }
            } else {
                overscrollAccum = 0
            }
        ...
        // Touch overscroll at section boundaries
        let touchStartY = null
        let touchOverscroll = 0
        const onTouchStartOverscroll = e => {
            if (!this.scrolled) return
            touchStartY = e.touches[0].clientY
            touchOverscroll = 0
        }
        const onTouchMoveOverscroll = e => {
            if (!this.scrolled || touchStartY === null) return
            const deltaY = touchStartY - e.touches[0].clientY
            const atBottom = this.viewSize - this.end <= 2
            const atTop = this.start <= 0
            if ((deltaY > 0 && atBottom) || (deltaY < 0 && atTop)) {
                touchOverscroll += Math.abs(deltaY)
                touchStartY = e.touches[0].clientY
                if (touchOverscroll > 60) {
                    triggerOverscroll(deltaY > 0)
                    touchStartY = null
                    touchOverscroll = 0
                }
            } else {
        ...
        this.#container.addEventListener('touchstart', onTouchStartOverscroll, { passive: true })
        this.#container.addEventListener('touchmove', onTouchMoveOverscroll, { passive: true })
        this.addEventListener('load', ({ detail: { doc } }) => {
            doc.addEventListener('touchstart', onTouchStartOverscroll, { passive: true })
            doc.addEventListener('touchmove', onTouchMoveOverscroll, { passive: true })
        })
